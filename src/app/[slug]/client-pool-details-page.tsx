...
interface ClientPoolDetailsPageProps {
  slug: string;
  initialPool: any; // Pool data from server component
}

export default function ClientPoolDetailsPage({
  slug,
  initialPool,
}: ClientPoolDetailsPageProps) {
  ...
  // Use your existing hook with initial data from server to prevent double fetch
  const {
    pool: rawPool,
    isLoading,
    error,
    mutate,
  } = usePoolDetailsV2({ slug }, { initialData: initialPool });
